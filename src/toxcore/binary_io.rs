...
pub trait FromBytes: Sized {
    ...
    named!(parse_bytes_multiple<&[u8], Vec<Self>>, many0!(Self::parse_bytes));

    /// De-serialize exact `times` entities from bytes.
    /// Note that even if `Vec<_>` is returned, it still can be empty.
    named_args!(parse_bytes_multiple_n(times: usize) <Vec<Self>>, many_m_n!(times, times, Self::parse_bytes));
